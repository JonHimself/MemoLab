...
// ANSWER:Yes, no need for numbers2
///SOLUTION
function getNumbersSquared(numbers) {
  let square = numbers
    .toString()
...
// ANSWER: Both function are doing the same thing, hopwever one uses a for loop and the other does .ForEach, however foreach is essentially the same thing as a for loop.  No extra memory has been created
...
class Factorial {
  ...
  constructor() {
    // Create a constructor that will hold the cache
    this.cache = {
      // create this.cache to an empty object
      answer: [1],
      // Add a key value pair of answer with a starting array of 1.
    };
  }

  calcFac(num) {
    // Create method calcFac with a param of num
    if (!this.cache.answer[num]) {
      // if the cached answer doesnt include num
      //run all of this.
      this.cache.answer[num] = num * this.calcFac(num - 1);
      // this.cache.answer[5]... 4...3..2..1
      // num 54321 * this.calcFac(num-1) recursion
    } else {
      // when the if is done, show that we hit no cache.
      console.log("cache hit:", num);
    }
    // return this.cache.answer to num.
    return this.cache.answer[num];
  }
}
...
console.log(factorial.calcFac(8));
console.log(factorial.calcFac(8));
console.log(factorial.calcFac(8));
console.log(factorial.calcFac(8));
...
class CacheMachine {
  ...
  cacheAnswers(answer) {
    this.cache.push(answer);
  }
}

let cashmoney = new CacheMachine();
cashmoney.cacheAnswers(10);
...
// CODE HERE
let cachearray = [];
const cacheAnswersAgain = (answer) => {
  cachearray.push(answer);
};

cacheAnswersAgain(10);
...
console.log(cacheAnswersAgain(10));
console.log(cacheAnswersAgain(10));
console.log(cacheAnswersAgain(10));
console.log(cacheAnswersAgain(10));
...
console.log(cashmoney.cacheAnswers(10));
console.log(cashmoney.cacheAnswers(5));
console.log(cashmoney.cacheAnswers(15));
console.log(cashmoney.cacheAnswers(20));
